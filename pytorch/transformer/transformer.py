...
class Transformer(nn.Module):
    def __init__(self, vocab_size: int, hidden_dim: int, num_heads: int, num_encoders: int, num_decoders: int):
        ...
        self.Encoder = Encoder(hidden_dim=hidden_dim, num_blocks=num_encoders, num_heads=num_heads)
...
class Encoder(nn.Module):
    def __init__(self, hidden_dim: int, num_heads: int, num_blocks: int):
        ...
        self.attention = MultiHeadAttention(hidden_dim=hidden_dim, num_heads=num_heads) # not implemented yet
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
        ...
        query = query.view(batch_size, -1, self.num_heads, self.hidden_dim_head).transpose(1, 2)
        key = key.view(batch_size, -1, self.num_heads, self.hidden_dim_head).transpose(1, 2)
        value = value.view(batch_size, -1, self.num_heads, self.hidden_dim_head).transpose(1, 2)
        attention_raw = F.softmax(torch.matmul(query, key.transpose(-1, -2)) / math.sqrt(self.hidden_dim))
        attention_score = torch.matmul(attention_raw, value).view(batch_size, -1, self.hidden_dim)
